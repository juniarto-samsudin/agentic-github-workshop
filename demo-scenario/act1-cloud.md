# ☁️ Act 1 — Cloud-Side Demo Scenarios

> **Agent があなたの代わりに非同期で自律実行する**
>
> チャプター 3〜8：GitHub.com 上で完結するデモシナリオ
>
> 📋 [全体構成に戻る](README.md)
...
Chat でコード理解 → 改善点の発見 → Issue として作成 → Copilot をアサイン
       ↓                  ↓                ↓                  ↓
  「説明して」       「テストがない」    Issue が作られる    チャプター 4 へ
```
...
🗣️ **トーク:**
*「Issue に Copilot をアサインするだけで、Agent が自律的にコードを書き始めます。ここから先はチャプター 4『実行』で詳しく見ていきます」*

> ⏳ **Agent の実行には 1〜3 分かかる。この間にチャプターの説明に戻り、チャプター 4 の説明を開始する。**
...
Issue アサイン済み → Actions で実行過程を観察 → セッションログ確認 → 完成 PR を確認
       ↓                    ↓                         ↓                    ↓
  チャプター 3 から      Agent の動きを見る        思考過程を追う       成果物レビュー
```
...
> ⏳ チャプター 3 で Issue に Copilot をアサイン済み。Agent が裏で動いている。
...
PR を開いている状態 → @copilot にフィードバック → Copilot が追加コミット → 反復を確認
       ↓                       ↓                         ↓                    ↓
  チャプター 4 から        エッジケース依頼          新しいコミット追加     タイムラインで可視化
```
...
2. タイムラインを上から下にスクロールして流れを見せる：
   ...
   ```
   [Copilot]  PR を作成（テスト一式）          ← チャプター 4
       ↓
   [人]       @copilot エッジケースを追加して    ← 今のステップ
       ↓
   [Copilot]  応答 + 新しいコミット追加
       ↓
   [人]       確認 → レビュー → マージ           ← チャプター 6 へ
   ```
...
- コメントを送った状態で「応答を待つ間に、次のチャプターに進みましょう」とチャプター 6 へ移行
- 後でチャプター 6 のデモ中に結果を確認することもできる
...
自動レビュー結果を確認 → コメント＆コード提案 → @copilot に修正依頼 → 新コミットで修正
       ↓                       ↓                       ↓                    ↓
  Copilot Review         バグ指摘・改善提案        レビュー→実行ループ    チャプター 4 に戻る
```
...
- Spaces が利用できない場合：
  - チャプターで概念を説明し「現在プレビュー提供中です」と補足

---

## 🔒 チャプター 8：統制（Governance）
...
2. **Mission Control のタスク一覧** を見せる：
   ...
   **今日のデモで作成したタスクが表示されていることを確認：**
   - 📋 チャプター 3 で作成した Issue（ユニットテスト追加）→ ステータス表示
   - 📋 チャプター 7 で作成した Issue（Custom Agent デモ用）→ ステータス表示
...
> チャプター 7 で新しい Issue をアサインしていれば、Agent が実行中の可能性がある。そうでなければ、新規タスクを作成する。
...
2. 複数の起動方法を説明：
   ...
   | 起動方法 | 場所 |
   |---------|------|
   | **Issue アサイン** | Issue ページで Copilot をアサイン（チャプター 3 で実演済み） |
